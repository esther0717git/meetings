...
import os
import sys
# Ensure helper modules are on path
...
def suggest_next_slot(room: str, start: datetime, duration: timedelta, pax: int):
    ...
    for i in range(1, 9):
        ...
        free_rooms = find_available_rooms(None, room, alt_start, duration, pax)
...
if st.button("Check Conflict & Suggest"):
    ...
    # 1) Conflict check
    conflict = get_conflicting_booking(None, room, start, duration)
    if conflict is None:
